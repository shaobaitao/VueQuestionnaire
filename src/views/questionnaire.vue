    ...
    <div class="content">
      ...
      <el-card class="box-card" v-for="(item,ques_index) in details " :key="ques_index">
        ...
        <div class="text item" v-for="(option,index) in item.options " :key="index">
          <el-radio v-if="item.type=='radio'" :label="option.id" style="margin: 5px;" v-model="item.radioValue">
            {{ option.title }}
          </el-radio>
        </div>
        <!--多选题展示-->
      ...
      <el-button type="primary" style="margin: 35px;" @click="submitForm">提交</el-button>
      <br>
